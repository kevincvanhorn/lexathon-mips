...
randomizeBoardLoop:
	...
	div $t3, $t3, 5 # Set range
	mfhi $t3
	abs $t3, $t3
	
	add $t6, $t7, $t3
	lb $t6, ($t6) # Load Character
	add $t7, $t7, 5 #thru queue
	
	add $t4, $t2, $t0 # store address of array[i] into $t4
	sb $t6, ($t4) # stores random num into array[index]
...
getCurTime: # Current Time into $s3
	...
	slti $t0, $a0, 0
	bne $t0, 1, timeContinue
	li $a0, 0 #Set negatives to 0
